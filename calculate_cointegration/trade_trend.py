...
def volaitlity_small(strategy, table):
    return strategy["up_open_time"] * table["stdev"] < strategy["cost_gate"]


def build_open(spread, table, strategy):
    spread_len = len(spread)
    slope = table["Johansen_slope"]
    up_open = table["mu"] + table["stdev"] * strategy["up_open_time"]  # 上開倉門檻
    down_open = table["mu"] - table["stdev"] * strategy["up_open_time"]  # 上開倉門檻
    up_stop_loss = table["mu"] + table['stdev'] * strategy["stop_loss_time"] 
    down_stop_loss = table["mu"] - table['stdev'] * strategy["stop_loss_time"] 
    close = table["mu"]  # 平倉(均值)
    up_open_list = []
    down_open_list = []
    close_list = []
    stop_loss_up_list = []
    ...
    for i in range(spread_len):
        ...
        stop_loss_up_list.append(up_stop_loss + (i+150) *slope)
        stop_loss_down_list.append(down_stop_loss + (i+150) *slope)  

    return close_list, up_open_list, down_open_list, stop_loss_up_list, stop_loss_down_list
...
def down_open(w1, w2, s1_price, s2_price, tax_cost):
    ...
    stock1_payoff, stock2_payoff = tax(stock1_payoff,tax_cost), tax(stock2_payoff,tax_cost)   # 計算交易成本
...
def trade_up_slope(s1_tick, s2_tick, table, strategy):
    ...
    spread = table["w1"] * s1_tick + table["w2"] * s2_tick
...
def trade_normal(s1_tick, s2_tick, table, strategy,formation_time,dump = False):
    trade_capital = 0
    cpA,cpB = 0,0
    trading_profit = 0.0
    trade = 0
    ...
    if trade > 0:  # 如果都沒有開倉，則報酬為0
        trade_return = trading_profit / trade_capital
    if dump and trading_profit < 0 :
            plot_spread( table["S1"], table["S2"], spread, 
                        trading_profit, pos, position,  table, strategy, formation_time)


    return trade, trading_profit, trade_capital, trade_return,trading_rule, history


path_to_image = "/Users/kuoweilun/CryptoCurrency_PT-main/negative_profit/"
def plot_spread( stock1, stock2, spread, local_profit, pos, position, table, strategy,formation_time):
    close, up_open_val, down_open_val, up_stop_loss_val, down_stop_loss_val = build_open(spread, table, strategy)
    plt.figure(figsize=(20, 10))
    plt.plot(spread)
    plt.vlines(formation_time,np.mean(up_open_val),np.mean(down_open_val))
    ...
    print(spread)
    spread = spread[formation_time:]
    spread = spread.reset_index(drop = True)
    print(spread)
    for x in range(1, len(pos)):
        if pos[x] != pos[x - 1] and pos[x] != 0:
            plt.scatter(x-1+formation_time, spread[x-1], color='', edgecolors='r', marker='o')
    ...
    plt.savefig(path_to_image+ '_' + str(stock1) + '_' + str(stock2) + '_' + 
                ' total profit: ' + str(local_profit) +'.png')
               # str(strategy["up_open_time"]) + '_' + str(strategy["up_open_time"]) + '_' + str(strategy["stop_loss_time"]) + '.png')
    plt.close('all')
